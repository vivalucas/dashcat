...
- **系统监控**
  - 四种监控模式：综合、CPU、内存、CPU 与内存数值
  - 猫咪动画速度实时反映系统压力——跑得越快，负载越高
  - 综合模式下自动取 CPU 和内存中压力更高者驱动动画
  - CPU 与内存数值模式以紧凑双行显示 C/M 百分比，不显示动画
  - 显示动画、显示数值两个开关可自由组合；至少保留一个开启
...
- **其他特性**
  ...
  - 可在当前 Finder 文件夹中新建 TXT 或 Markdown 文件，创建前会显示目标路径，也可临时改选文件夹
  - 支持开机启动
...
- **右键点击**猫咪图标：打开设置菜单
  ...
  - 开关极简电量显示、设置接电时隐藏
  - 在当前 Finder 文件夹中新建文件、反转鼠标滚轮、切换语言、开关动画/数值显示、设置开机启动
...
DashCat 只在你点击“在当前 Finder 文件夹中新建文件”时读取 Finder 当前窗口目录。macOS 可能会弹出自动化权限请求，用来允许 DashCat 获取这个路径；平时不会常驻监听 Finder。这个入口在 DashCat 菜单里，不会注入 Finder 空白处右键菜单。
