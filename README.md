...
- **剪贴板管理**
  - 左键点击猫咪图标，弹出剪贴板面板
  - 搜索框实时过滤
  - 方向键选择，Enter 复制；文本始终以纯文本回写，Option + Enter 保持兼容。
  - 右键条目可 Pin 固定到顶部
  - 支持完整纯文本和图片（可选保存原始 PNG/TIFF，缩略图单独生成）
  - 历史支持“加载更多”和右键预览。可在剪贴板设置中选择打开面板快捷键（默认关闭）。敏感及临时剪贴板标记会被忽略；过期记录每小时及唤醒后清理，固定项除外。
...
- **极简电量显示**
  ...
  - 不显示百分号，只用极窄数字加浅蓝色电量底纹，适合刘海屏和拥挤菜单栏
  - 使用电池时，20% 及以下显示橙色数字/描边，10% 及以下显示红色数字/描边
  - 接电或充电时切换为不增宽的粉色底纹/描边
...
- **其他特性**
  ...
  - 可在 Finder 中新建 TXT 或 Markdown 文件，创建前可输入文件名、查看目标路径，也可临时改选文件夹
  - 支持开机启动
...
- **右键点击**猫咪图标：打开设置菜单
  ...
  - 在剪贴板设置中管理图片保存、保留天数、过滤词和清除历史，清除时可选择保留固定项或清除全部
  - 开关极简电量显示、设置接电时隐藏
...
`~/Library/Application Support/DashCat/` 目录下。`clipboard.db` 存文本记录，`Images/` 文件夹存图片。清除历史时会先确认，可只清除未固定项，也可以连固定项和图片一起清除。

**图片存储会占多少空间？**

图片默认不保存。开启后保留原始 PNG/TIFF 数据和分辨率，空间取决于原图大小。图片及缩略图合计以 500MB 为清理阈值，超出后删除最旧的未固定图片；固定图片始终保留，因此可能超过阈值。旧版已压缩的图片无法恢复原始质量。
...
DashCat 只在你点击“在 Finder 中新建文件”时读取 Finder 当前窗口目录。macOS 可能会弹出自动化权限请求，用来允许 DashCat 获取这个路径；平时不会常驻监听 Finder。这个入口在 DashCat 菜单里，不会注入 Finder 空白处右键菜单。创建前可以输入文件名、选择 TXT 或 Markdown，并在需要时临时改选文件夹。
